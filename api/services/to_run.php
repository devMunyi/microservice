...
$ms = fetchtable("tbl_services", "DATE_FORMAT(next_run_datetime, '%Y-%m-%d %H:%i') = '$current_fulldate' AND is_executed = 'No' AND status > 0", "id", "DESC", "100", "id, service_title, service_address, last_run_datetime, next_run_datetime, unit, frequency, is_executed, repeated");
///----------Paging Option

$alltotal = countotal("tbl_services", "DATE_FORMAT(next_run_datetime, '%Y-%m-%d %H:%i') = '$current_fulldate' AND is_executed = 'No' AND status > 0");
